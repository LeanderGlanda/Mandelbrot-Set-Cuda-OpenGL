...
void setUpCuda(GLuint PBO)
{
    cudaGLSetGLDevice(0);
    cudaGLRegisterBufferObject(PBO);
}

void launchCuda(GLuint PBO)
{
    void* devicePtr = nullptr;
    cudaGLMapBufferObject(&devicePtr, PBO);

    // Create the kernel launch size
    dim3 hblockDim = dim3(16, 16, 1);
    dim3 hgridDim = dim3(width / hblockDim.x, height / hblockDim.y, 1);

    // Launch the kernel
    mandelbrotLogicKernel << <hgridDim, hblockDim >> > ((float*)devicePtr, width, height, maxIteration, middlea, middleb, rangea, rangeb);
